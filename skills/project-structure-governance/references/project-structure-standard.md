...
测试执行产生的可复核截图、浏览器网络/控制台摘要和文本附件是测试报告资产，唯一归档根为 `docs/testing/{feature-slug}/test-report/evidence/{date}-{slug}/`。每个证据集必须包含 `001-overview.md`，并按 `screenshots/`、`logs/`、`attachments/` 分类；不得迁入设计截图目录，也不得把根 `evidence/` 或 `.run/` 当作正式来源。证据文件必须使用三位序号和受控类型命名，原始 HAR、完整响应体、cookie、token、测试账号、邮箱、手机号及其他 PII 不得归档；网络与控制台输出只保留完成复核所需的脱敏摘要。证据索引必须记录关联测试报告、环境、采集时间、执行者角色、文件清单、脱敏检查和可达性结论；不可达证据不得支持发布通过结论。
